  ...
  try {
    ...
    if (!schoolCode) {
      ...
      const { data: student } = await supabase
        .from("students").select("*").eq("gmail", gmail.toLowerCase().trim()).maybeSingle();
      ...
      if (student.password) {
        const ok = isLegacyPlain(student.password)
          ? student.password === password
          : await bcrypt.compare(password, student.password);
        if (!ok) return Response.json({ error: "Нууц үг буруу байна" }, { status: 401 });
        // хуучин plain-text байвал hash болгоно
        if (isLegacyPlain(student.password)) {
          const hashed = await bcrypt.hash(password, 10);
          await supabase.from("students").update({ password: hashed }).eq("gmail", student.gmail);
        }
      }
    ...
    if (!existing) {
      ...
      const updates = {
        password: hashed, grade: Number(grade),
        class_section: classSection || "А",
        last_name: lastName.trim(), first_name: firstName.trim(),
        teacher_gmail: teacherGmail,
      };
      if (isExpired(existing.joined_at)) { updates.joined_at = new Date().toISOString(); updates.personal_code = generateCode(); }
    ...
    return Response.json({
      mode: "registered", student: toJS(student),
...
function toJS(s) {
  return {
    gmail: s.gmail, lastName: s.last_name, firstName: s.first_name,
    code: s.code, personalCode: s.personal_code,
    grade: s.grade, classSection: s.class_section,
    teacherGmail: s.teacher_gmail, joinedAt: s.joined_at,
  };
}
